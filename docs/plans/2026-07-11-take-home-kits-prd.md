...
- [ ] [DATA] Catalog item `Rental Deposit` — **configurable per tier** (variations): launch values $50 / $75 / $100 for serves-10/15/20 (deposit scales with what's out the door); added as a line item only when the selected package includes rental pieces; amounts live in `kit-content.ts` + catalog, adjustable without code
...
- [ ] [LOGIC] **Retrieval option**: for overdue rentals the studio actually needs back, staff may arrange retrieval from the customer's provided address (friendly framing: "hey, we're swinging by to pick these up"); a configurable retrieval fee (`retrievalFeeCents`, launch $25) is deducted from the deposit; logged in the kit custody log; covered by agreement §6a(b)
...
- **Inventory model — weekly settings ledger (DECIDED 2026-07-11)**: `kit-content.ts` holds, per theme, `ownedSettings` (e.g. 45) and `heroSets` (cake stand + tray groups, e.g. 3). Availability is COMPUTED PER WEEK, never decremented: for any Thu→Wed week W, `available = ownedSettings − Σ(tier of every paid kit/party order pinned to W)`, and concurrent uses in W must also be ≤ `heroSets`. A booking 8 weeks out only affects that week's sum; when the week passes, the settings are automatically free for all future weeks — no restock step, no counter to maintain. Source of truth = the orders already in Square (same pattern as party availability today). **LAUNCH CONFIG (Kaden 2026-07-11)**: two deep themes — **The Gilded Table** and **The Prism Table** — physically stocked at **60 settings each** while the ledger advertises only **45** (`ownedSettings: 45`): the 15-setting gap is an invisible shock absorber for non-returns, breakage, and slow wash days, sized against a >1-week reorder lead time. 3 hero sets each. **The Sweet Sixteen launches as a styling variant of Gilded** (its own consumables/candles/staging, shared tableware — a third menu item for ~$100 of consumables, not $1,200 of plates). Sterling/Bluebell/Linen list on the site as **waitlist-only** (existing notify-me pattern) — longest waitlist = next theme purchased.
- **Late/damaged protection**: a kit's settings stay "consumed" from pickup until staff CHECKS IT IN (not until the scheduled Wednesday) — an overdue kit automatically keeps blocking the following week's math, surfacing conflicts instead of hiding them. Staff can also manually block a theme-week (damage, deep cleaning). Breakage shrinks `ownedSettings` via config edit when it ever matters.
...
- [ ] [LOGIC] **Shared inventory pool with kits**: in-studio themed parties draw from the same weekly settings ledger (§10) — a party on date D pins its tier to the Thu→Wed week containing D; offered iff remaining settings ≥ tier and a hero set is free. Both flows read the same computation, so a kit booked 8 weeks out correctly reserves against that future week
